...
// ПРИМЕЧАНИЕ: Это очень простая и неоптимизированная реализация
class Conv2DLayer : public Layer {
public:
    Tensor kernels; // Веса (ядра)
...
public:
    Conv2DLayer(int input_channels, int output_channels, int kernel_size, int stride = 1, int padding = 0)
        ...
        biases = Tensor({1, out_channels}); // Используем 1D, но храним как 2D
        
        std::default_random_engine generator;
        // Более подходящая инициализация для сверточных слоев (He/Xavier)
    ...
    Tensor forward(const Tensor& input) override {
        last_input = input;
        // Ожидаемая форма входа: {N, C_in, H_in, W_in}
    ...
    Tensor backward(const Tensor& output_gradient) override {
        // Backward pass здесь будет очень сложным для реализации "в лоб"
        // и выходит за рамки простого примера.
        // Он требует вычисления градиентов по входу, весам и смещениям.
        // Для простоты оставим его нереализованным.
        // В реальных фреймворках это делается через сложные операции (im2col, транспонированные свертки).
        grad_kernels = Tensor(kernels.shape); // Заглушка
        grad_biases = Tensor(biases.shape);   // Заглушка
        return Tensor(last_input.shape);      // Заглушка
    }
};