...
private:
    Tensor last_output; // Сохраняем выход для вычисления градиента
...
public:
    Tensor forward(const Tensor& input) override {
        ...
        last_output = output; // Производная сигмоиды выражается через ее значение
        return output;
    }
    ...
    Tensor backward(const Tensor& output_gradient) override {
        ...
        for (size_t i = 0; i < last_output.data.size(); ++i) {
            ...
            // Умножаем градиент на производную сигмоиды: s * (1 - s)
